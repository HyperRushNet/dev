(function (window) {
    ...
    class HRNGame extends HTMLElement {
        ...
        connectedCallback() {
            ...
            // Check for "false" string specifically
        ...
        render() {
            ...
            const css = `
                :host { display: block; width: 100%; height: 100%; position: relative; background: #000; overflow: hidden; }
                ...
                iframe { width: 100%; height: 100%; border: none; display: block; background: #000; position: absolute; top: 0; left: 0; }
        ...
        async loadGame() {
            ...
            // Base sandbox permissions
            const sandbox = ["allow-scripts", "allow-same-origin", "allow-pointer-lock", "allow-forms"];
            
            // If redirectblock is FALSE (meaning: allow redirects), add navigation permissions
            if (!this.redirectblock) {
                sandbox.push("allow-popups");
                sandbox.push("allow-popups-to-escape-sandbox");
                sandbox.push("allow-top-navigation"); // Nodig om de hoofdpagina te veranderen
                sandbox.push("allow-top-navigation-by-user-activation");
            }

            pc.innerHTML = `<iframe src="${url}" allow="autoplay; fullscreen; gamepad" sandbox="${sandbox.join(" ")}" scrolling="no"></iframe>${this.credits ? `<div class="credits">Powered by <a href="https://hyperrushnet.github.io/" target="_blank" rel="noopener">HyperRush</a></div>` : ""}`;
    ...
    class HRNGamePortal extends HTMLElement {
        ...
        render() {
            ...
            const scrollbarCSS = `
                .w { -ms-overflow-style: none; scrollbar-width: none; }
                .w::-webkit-scrollbar { display: none; }
            `;

            const css = `
                :host { display: block; position: relative; width: 100%; height: ${height}; background: ${bg}; color: ${fg}; font-family: sans-serif; overflow: hidden; border: 1px solid ${border}; }
