(function (window) {
    ...
    async function fetchGames() {
        ...
        ongoingFetch = fetch(GAME_JSON_URL)
            .then(r => r.ok ? r.json() : Promise.reject("Games fetch failed"))
            .then(data => {
    ...
    window.hrn.stats = window.hrn.stats || {
        get: () => {
    ...
    class HRNGame extends HTMLElement {
        ...
        connectedCallback() {
            ...
            if (!this.nr) return console.error("<hrn-game> missing required 'nr' attribute.");
        ...
        render() {
            ...
            const loaderColor = accent.trim();

            const css = `
                :host { display: block; width: 100%; height: 600px; position: relative; }
                .ldr { width: 40px; height: 40px; border: 4px solid #ccc; border-top-color: ${loaderColor}; border-radius: 50%; animation: spin 1s linear infinite; position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); }
                ...
                .credits a { color: ${accent}; text-decoration: none; font-weight: 700; }
    ...
    class HRNGamePortal extends HTMLElement {
        ...
        render() {
            ...
            const css = `
                ...
                .msg { display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%; }
                ...
                @keyframes spin { to { transform: rotate(360deg); } }
                .err-msg { color: #ff6b6b; font-size: 1.1rem; text-align: center; padding: 1.5rem; }
                ...
                .g { display: grid; grid-template-columns: repeat(auto-fill, minmax(min(100%, ${cardSize}), 1fr)); width: 100%; margin-top: -75px; }
                .c { aspect-ratio: 1/1; border: 1px solid ${border}; display: flex; flex-direction: column; align-items: center; justify-content: center; cursor: pointer; transition: .2s; text-align: center; padding: 20px; margin: -0.5px; }
        ...
        async initialize() {
            ...
            const toggle = () => {
                if (sh.classList.toggle("active")) q.focus();
            };
            ...
            const render = games => {
                if (!games.length) { grid.innerHTML = '<div class="msg">No games found</div>'; return; }
    ...
    // --- Database helper ---
        ...
        db: {
            ...
            search: async (query = "") => {
                ...
                return games
                    .filter(g =>
                        ...
                        (g.device ?? []).some(d => norm(d).includes(q))
                    )
                    .sort((a, b) => {
                        const pa = norm(a.name).indexOf(q);
                        const pb = norm(b.name).indexOf(q);
            ...
            find: async criteria => {
                ...
                return games.filter(g => Object.entries(criteria).every(([k,v]) => {
                    if (Array.isArray(g[k])) return g[k].includes(v);
                    return g[k] === v;
                }));
            },
            random: async () => {
        ...
        stats: {
            ...
            getTop: async (limit = 5) => {
                const s = window.hrn.stats.get();
                const topIds = Object.entries(s.counts || {})
                    .sort(([,a],[,b]) => b-a)
                    .slice(0, limit)
                    .map(([id]) => Number(id));
                const games = await fetchGames();
