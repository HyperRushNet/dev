(function (window) {
  ...
  const toLowerTrim = str => String(str ?? "").toLowerCase().trim();
  ...
  window.hrn = {
    db: {
      ...
      search: async (query = "") => {
        ...
        const term = toLowerTrim(query);
        if (!term) return [...games];

        return games
          .filter(g =>
            toLowerTrim(g.name).includes(term) ||
            toLowerTrim(g.category ?? "").includes(term) ||
            (g.device ?? []).some(d => toLowerTrim(d).includes(term))
          )
          .sort((a, b) => {
            const posA = toLowerTrim(a.name).indexOf(term);
            const posB = toLowerTrim(b.name).indexOf(term);
...
`;
    ...
    async initialize() {
      ...
      const grid = root.getElementById("games-grid");
      const input = root.getElementById("search-input");
      const wrapper = root.getElementById("search-wrapper");
      const overlay = root.getElementById("overlay");
      const player = root.getElementById("player-container");
      const main = root.getElementById("main-wrapper");
      const loading = root.getElementById("loading");
      const errorEl = root.getElementById("error-state");
      ...
      root.getElementById("search-toggle").onclick = toggleSearch;
      root.getElementById("search-toggle").onkeydown = e => {
      ...
      root.getElementById("close-btn").onclick = () => {
      ...
      const renderGames = (games) => {
        ...
        grid.querySelectorAll(".c").forEach(card => {
          const openGame = () => {
            player.innerHTML = `<hrn-game nr="${card.dataset.id}" provider="${provider}" credits="${showCredits}"></hrn-game>`;
            overlay.classList.add("active");
            main.classList.add("lock");
            main.scrollTo({ top: 0, behavior: "smooth" });
          };

          card.onclick = openGame;
          card.onkeydown = e => {
            if (e.key === "Enter" || e.key === " ") {
              e.preventDefault();
              openGame();
      ...
      try {
        const all = await window.hrn.db.all();
        loading.remove();
        renderGames(all);
        ...
        input.oninput = () => {
          ...
          this._debounceTimer = setTimeout(async () => {
            const results = await window.hrn.db.search(input.value);
            renderGames(results);
          }, 300);
        };
      } catch {
  ...
  class HrnGame extends HTMLElement {
    ...
    async load() {
      const nr = this.getAttribute("nr");
      const provider = this.getAttribute("provider") || "https://hyperrushnet.github.io";
      const showCredits = this.getAttribute("credits") !== "false";
      const strictRedirect = this.getAttribute("redirectblock") !== "false";

      let gameUrl = `${provider}/games-1/404.html`; 

      try {
        const games = await fetchGames();
        const match = games.find(g => Number(g.number) === Number(nr));
        if (match?.link) gameUrl = provider + match.link;
      } catch {
      ...
      if (!strictRedirect) {
        sandbox.push("allow-popups", "allow-popups-to-escape-sandbox");
...
<div class="container">
  ...
  <iframe class="frame" 
      ...
      const loaderEl = this.shadowRoot.getElementById("load-screen");
